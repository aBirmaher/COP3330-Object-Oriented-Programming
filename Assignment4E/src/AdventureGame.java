...
//Edited by: Alan Birmaher
//COP 3330-001
//3/27/13
//Purpose: To run our adventure game, in a modified manner.
...
public class AdventureGame implements KeyListener,MouseListener {
	...
	//Keep track of the amount of moves
	...
	//Keeps track of the number of characters we have
	public int charNum=3;
	
	//Keps track of how many times the treasure has been collected
	public int collected=0;
	
	//New score counter label created
	...
	private void buildGui() {
		...
		//Add the score bar
		scoreBar= new JLabel();
		scoreBar.setBorder(BorderFactory.createLoweredBevelBorder());
	...
	private void newGame() {
		...
		//Increases the game count
		gameCount++;
		
		//Starts the player off with 3 lives
		liveCount=3;
	...
	/** Show the given message on the status bar and score bar. */
	private void updateStatus(String msg) {
		statusBar.setText(msg);
		scoreBar.setText("Moves: "+moveCount+"									Game #: "
				+gameCount+"									Extra Lives: "+liveCount+
				"									Characters Remaining: "+charNum+
				"									Times Won: "+collected);	}
	...
	public void keyPressed(KeyEvent e) {
		...
		if (gameBoard.ok(c.getRow()+dr,c.getCol()+dc)) {
			...
			//If the place we want to go is a pit and we have lives to give we don't move there just lose a life
			else if (newC.isPit()&&liveCount>0&&ch.getName()!="Filler") {
				
				//Decreases one life
				liveCount--;
				
				//Prints a message to the user
				updateStatus(ch.getName()+" fell in the pit and lost a life for the team! Only "+liveCount+" left!");
			...
			// Try and make the move.
				...
				else if (newC.isPit()) { // This character falls in the pit.
					
					//Reduces live count to 0 if miner dies
					if(ch.getName()=="Miner")
						liveCount=0;
					
					//Reduces the number of players by 1 every time somoene dies
					charNum--;
				...
				if (!treasureClaimed && newC.getRow()==DEFAULT_ROWS-1 && 
					...
					//Adds to the amount of times a treasure has been collected
					collected++;
